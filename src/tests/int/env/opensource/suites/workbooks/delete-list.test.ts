...
describe('Deleting empty workbooks in the root', () => {
    ...
    test('Get deleted workbooks error', async () => {
        await auth(request(app).get(`${routes.workbooks}/${emptyRootWorkbook.workbookId}`)).expect(
            404,
        );

        await auth(request(app).get(`${routes.workbooks}/${emptyRootWorkbook2.workbookId}`)).expect(
            404,
        );
    });
});
...
describe('Deleting workbooks within collection and with content', () => {
    ...
    test('Get deleted workbooks error', async () => {
        await auth(
            request(app).get(`${routes.workbooks}/${workbookWithinCollection.workbookId}`),
        ).expect(404);

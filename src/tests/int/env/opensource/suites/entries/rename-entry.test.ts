...
import {app, appNodekit, auth, testTenantId} from '../../auth';
...
describe('Rename entries', () => {
    ...
    test('Rename folder', async () => {
        const {body} = await auth(request(app).post(`${routes.entries}/${folderEntryId}/rename`))
            .send({
                name: newFolderName,
            })
            .expect(200);
    ...
    test('Rename entry in workbook by Viewer should fail with 403', async () => {
    ...
    test('Rename entry in Folder', async () => {
        ...
        const {body} = await auth(request(app).post(`${routes.entries}/${entryInFolderId}/rename`))
            .send({
                name: newName,
            })
            .expect(200);

...
import {DlsActions, DlsEntity, EntryScope} from '../types/models';
...
type GetEntriesWithPermissionsOnlyArgs = {
    entries: Entries[];
    includePermissionsInfo?: boolean;
    isPrivateRoute?: boolean;
};

export const getEntriesWithPermissionsOnly = async <T = []>(
    ctx: AppContext,
    {entries, includePermissionsInfo, isPrivateRoute}: GetEntriesWithPermissionsOnlyArgs,
): Promise<T> => {
    const workbookEntries: Entries[] = [];
    const entryWithoutWorkbook: Entries[] = [];
    let result: any[] = [];

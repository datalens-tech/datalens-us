...
import {bulkFetchCollectionEntryPermissions, createCollectionEntry} from './collection-entry';

export interface DeleteCollectionEntriesArgs {
    entryIds: string[];
...
export const deleteCollectionEntries = async (
    ...
    const entries = await queryPrimary(Entry, mainTrx)
        ...
        .whereIn([EntryColumn.Scope], ALLOWED_COLLECTION_ENTRY_SCOPES)
    ...
    if (accessServiceEnabled && !skipCheckPermissions && !isPrivateRoute) {
        const instancesMap = await bulkFetchCollectionEntryPermissions({ctx}, items);

        const accessDenied = [...instancesMap.values()].some(
            (instance) => !instance.hasPermission(CollectionEntryPermissions.Delete),
        );
    ...
    const deletePermissions = async () => {
        await Promise.all(
            deletedEntries.map(async (entry) => {
                ...
                const instance = createCollectionEntry(ctx, item.model);
    ...
    ctx.log('DELETE_COLLECTION_ENTRIES_FINISH');
